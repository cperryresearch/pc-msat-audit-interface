...
def compute_signed_curvature(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    No smoothing, no filtering. Endpoints handled by edge padding.
    """
    ...
    # Avoid divide-by-zero: where denom==0, set kappa=0
...
def state_boundaries(states: np.ndarray) -> List[int]:
    """
    Boundary indices where state changes. We mark vertical ticks at i where states[i] != states[i-1].
    """
    b = []
    for i in range(1, len(states)):
...
def draw_curvature_panel(ax, kappa: np.ndarray, boundaries: List[int]) -> None:
    ...
    # κ=0 reference line (slightly stronger than grid)
    ax.axhline(0.0, linestyle="-", linewidth=0.9, alpha=0.35)

    # State boundary ticks (faint vertical lines)
    for b in boundaries:
        ax.axvline(b, linestyle="-", linewidth=0.7, alpha=0.30)

    # Curvature signal on top
    ax.plot(idx, kappa, linewidth=1.4, color="#222222")

    # Symmetric y-limits around 0 with a small margin
...
def draw_persistence_panel(
    ...
    ax_top.invert_yaxis()  # Run 1 at top

    # ---- Diagnostic layer (state-typed) ----
    # Aggregate run lengths by state (sum of run lengths per state, preserving count info isn’t required here)
    # We keep it simple: show each run as its own bar but label by state (still diagnostic)
    diag_states = [s for (_, s, _, _) in runs]
    diag_lengths = [length for (_, _, _, length) in runs]
    y2 = np.arange(len(diag_lengths))

    colors = [SOD_COLORS[s] for s in diag_states]
    ax_bottom.barh(y2, diag_lengths, color=colors, alpha=0.95)
    ax_bottom.axvline(min_run, color="#444444", linewidth=1.0, alpha=0.6)
    ax_bottom.set_yticks(y2)
    ax_bottom.set_yticklabels(diag_states, fontsize=8)
    ...
    # Footnote-like note (in bottom axis)
    ax_bottom.text(
        0.0,
        -0.35,
...
def render_audit_sheet(
    ...
    # Integrity check: refuse mismatch
    if result != meta.case_label.upper():
        raise ValueError(
            f"CASE_LABEL mismatch: declared={meta.case_label.upper()} computed={result} (max_run={max_run}, MIN_RUN={meta.min_run})"
        )
    ...
    # Main grid: header + 4 panels
    # Header gets a small fixed fraction via explicit top margin; use a dedicated axis for header text.
    gs = GridSpec(
        nrows=5,
        ncols=1,
        height_ratios=[0.9, 4, 2, 2, 1],  # header, P1, P2, P3, P4
        hspace=0.55,
    )
    ...
    # Panel 3: persistence (split into two rows inside the panel)
    gs_p3 = gs[3].subgridspec(2, 1, height_ratios=[1, 1], hspace=0.5)
