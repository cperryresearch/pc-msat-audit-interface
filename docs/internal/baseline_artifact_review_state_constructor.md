...
| 7 | `state_constructor/output/.gitkeep` | output placeholder | keeps output directory tracked for historical/generated-output layout | low risk placeholder, but durable tracking of `state_constructor/output/` should be reconsidered because generated outputs are now ignored by policy | keep temporarily grandfathered pending output-directory cleanup | keep temporarily grandfathered pending output-directory cleanup | Revisit in a separate cleanup step after deciding whether generated output directories should remain tracked at all. |
